...
def get_extractive_summary(input_type, file_name, limit_type, limit):
    sample_text = generate_input_text(input_type, file_name)
    if(limit_type=='sentence' or limit_type=='word'):
...
def input_txt_length(input_type,file_name):
    input_text = generate_input_text(input_type, file_name)
    preprocessed_sentences = preprocess_input_text(input_text)
    text_sent_count = len(preprocessed_sentences)
    text_word_count = len(input_text.split())

    return input_text , text_sent_count , text_word_count




def generate_input_text(input_type, file_name):
    if input_type == 'audio' or input_type == 'video':
        input_text = get_transcripts(file_name)
    elif input_type == 'text':
        print(BASE_DIR)
        file = open(os.path.join(BASE_DIR,'media\\documents',file_name),"r",encoding="utf-8")
        input_text = file.read()
    else: 
        raise Exception("Invalid Input Type. Try 'text','video' or 'audio'.")
    return input_text
...
word_limit = 0
sentence_limit = 5
limit_type = 'sentence' #'sentence' or 'word'
input_type = 'text' #'audio', 'video or 'text'
input_file_name = 'inputtext.txt'
input_text = generate_input_text(input_type, input_file_name)
'''

#text_sen, text_words = input_txt_length(input_type,input_file_name)

""" Generate Summary """
#extractive_summary, text_word_count, text_sent_count, summ_sent_count, summ_word_count = get_extractive_summary(input_type, input_file_name, limit_type, sentence_limit)
